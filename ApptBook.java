...
/******************************************************************************
 * This class is a homework assignment; An ApptBook ("book" for short) is a
 * sequence of Appointment objects in sorted order. The book can have a special
 * "current element," which is specified and accessed through four methods that
 * are available in the this class (start, getCurrent, advance and isCurrent).
 ******************************************************************************/
public class ApptBook implements Cloneable {
	...
	/**
	 * Return true if the given subtree has height no more than a given bound. In
	 * particular if the "tree" has a cycle, then false will be returned since it
	 * has unbounded height.
	 * 
	...
	/**
	 * Return whether all the nodes in the subtree are in the given range, and also
	...
	private boolean allInRange(Node r, Appointment lo, Appointment hi) {
		...
		if (lo != null && r.data.compareTo(lo) < 0)
			return report("Out of Range");
		if (hi != null && r.data.compareTo(hi) >= 0)
			return report("Out of Range");
		Appointment app = r.data;
		return allInRange(r.left, lo, app) && allInRange(r.right, app, hi);
	}

	/**
	 * Return whether the cursor was found in the tree. If the cursor is null, it
	 * should always be found since a binary search tree has many null pointers in
	 * it. This method doesn't examine any of the data elements; it works fine even
	 * on trees that are badly structured, as long as they are height-bounded.
	 * 
	...
	private boolean wellFormed() {
		...
		if (manyItems != countNodes(root))
			return report("ManyItem not equal to number of node");
	...
	/**
	 * Return the first node in a non-empty subtree. It doesn't examine the data in
	 * teh nodes; it just uses the structure.
	 * 
	 ...
	 * @param acceptEquivalent whether we accept something equivalent. Otherwise,
	 *                         only appointments after the appt are accepted.
	 ...
	 * the guide. This operation will be efficient if the tree is balanced.
	 * 
	 ...
	 *                current el;ement (if any) is unchanged.
	 **/
